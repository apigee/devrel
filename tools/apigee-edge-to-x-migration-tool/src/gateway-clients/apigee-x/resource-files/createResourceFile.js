 ...
 * @returns {Promise<*>}
 */
export default async function createApigeeXResourceFile(apigeeXClient, environment, type, name, filePath) {
  const query = {
    type,
    name
  };
  ...
  return apigeeXClient.post(`/environments/${environment}/resourcefiles?${querystring.stringify(query)}`, fileData);
}
