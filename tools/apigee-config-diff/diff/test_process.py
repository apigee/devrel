...
def test_process_files_dry_run(tmp_path, capsys):
    ...
    # update/src/org1/env/dev/file.json
    # update/src/org1/org/file.json
...
def test_process_files_confirm_env(tmp_path):
    ...
    with patch("subprocess.run") as mock_run:
        process_files(str(tmp_path), "src", confirm=True)
...
def test_process_files_empty_action_dir(tmp_path, capsys):
    (tmp_path / "update" / "src").mkdir(parents=True)
    process_files(str(tmp_path), "src", confirm=False)
    out, _ = capsys.readouterr()
...
def test_process_files_edge_cases(tmp_path, capsys):
    ...
    (tmp_path / "update" / "src").mkdir(parents=True)
    (tmp_path / "update" / "src" / "invalid").write_text("{}")
    process_files(str(tmp_path), "src", confirm=False)
    out, _ = capsys.readouterr()
    ...
    with patch("os.walk") as mock_walk:
        mock_walk.return_value = [(str(tmp_path / "update" / "src"), [], ["file.json"])]
        process_files(str(tmp_path), "src", confirm=False)
