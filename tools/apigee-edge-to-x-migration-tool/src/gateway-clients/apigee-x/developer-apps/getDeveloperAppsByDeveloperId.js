 ...
 * @returns 
 */
async function getApigeeXDeveloperAppsByDeveloperId(apigeeXClient, developerEmail, callback = null, startKey = null) {
  const limit = 100; // Max results that Apigee public cloud can return at once
  ...
  return apigeeXClient.get(`/developers/${developerEmail}/apps?${querystring.stringify(query)}`).then(async response => {
    ...
    if (lastAppId && lastAppId.length > 0) {
      ...
      return getApigeeXDeveloperAppsByDeveloperId(apigeeXClient, developerEmail, callback, lastAppId).then(results => developerApps.concat(results));
    }
...
export default getApigeeXDeveloperAppsByDeveloperId;