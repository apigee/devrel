...
def main():
    ...
    proxy_targets = {}
    if check_proxies:
        skip_proxy_list = cfg['validation'].get('skip_proxy_list', '').split(',')  # noqa
        print('INFO: exporting proxies to be analyzed ! this may take a while !')  # noqa
        ...
        for each_api_type in api_types:
            ...
            api_revision_map[each_api_type]['export_dir'] = proxy_export_dir+f'/{each_api_type}'  # noqa
            ...
            for each_api in SourceApigee.list_apis(each_api_type):
                if each_api not in skip_proxy_list:
                    api_revision_map[each_api_type]['proxies'][each_api] = SourceApigee.list_api_revisions(each_api_type,each_api)[-1]  # noqa
                else:
        ...
        for each_api_type, each_api_type_data in api_revision_map.items():
            ...
            for each_api, each_api_rev in each_api_type_data['proxies'].items():  # noqa
                print(f"Exporting API : {each_api} with revision : {each_api_rev} ")  # noqa
                SourceApigee.fetch_api_revision(each_api_type,
                                                each_api,
                                                each_api_rev,
                                                api_revision_map[each_api_type]['export_dir'])  # noqa
                print(f"Unzipping API : {each_api} with revision : {each_api_rev} ")  # noqa
                unzip_file(
                ...
                for each_te in proxy_tes:
                    if each_te in proxy_targets:
                        proxy_targets[each_te].append(f"{each_api_type} - {each_api}")  # noqa
                    else:
                        proxy_targets[each_te] = [f"{each_api_type} - {each_api}"]  # noqa
    ...
    print(f"INFO: Fetching VHost with name {cfg['validation']['vhost_domain_name']} !")  # noqa
    ...
    for each_ts in all_target_servers:
        ...
        final_report.append([
            ...
            each_ts['env'],
            status,
            " & ".join(list(set(proxy_targets[each_ts['name']]))) if each_ts['name'] in proxy_targets else "No References in any API"  # noqa
        ])
    ...
    for each_api_type, apis in proxy_hosts.items():
        for each_api, each_targets in apis.items():
            for each_target in each_targets:
                if not has_templating(each_target['host']) and not each_target['target_server']:  # noqa
                    if f"{each_target['host']}:{each_target['port']}" in _cached_hosts:  # noqa
                        print('INFO: Fetching validation status from cached hosts')  # noqa
                        status = _cached_hosts[f"{each_target['host']}:{each_target['port']}"]  # noqa
                    else:
                        status = run_validator_proxy(
                            api_url,
                            vhost_domain_name,
                        ...
                        _cached_hosts[f"{each_target['host']}:{each_target['port']}"] = status  # noqa
                    final_report.append([
                        each_api,
                        "APIProxy" if each_api_type == 'apis' else "SharedFlow",  # noqa
                        each_target['host'],
