...
class MavenDeployer:
    ...
    def _run_mvn(self, action: str, config_dir: str, org: str, env: str = None):
        cmd = [
            "mvn", "install", f"-P{org}",
...
def get_affected_orgs_and_envs(action_base_path: str, affected_files: list) -> dict:
    """
    Parses the affected files to find the involved organizations and environments.
    Returns a dictionary mapping organization names to a set of environment names.
    """
...
def process_files(output_base_path, resources_folder, confirm, bearer=None, sa_path=None):
    ...
    for action in actions:
        ...
        org_env_map = get_affected_orgs_and_envs(action_base_path, affected_files)
