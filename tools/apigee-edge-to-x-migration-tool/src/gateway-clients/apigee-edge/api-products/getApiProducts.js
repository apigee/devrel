 ...
 */
export default async function getApigeeEdgeApiProducts(apigeeEdgeClient, callback = null, startKey = null) {
  const query = {
    expand: true,
  ...
  return apigeeEdgeClient.get(`/apiproducts?${querystring.stringify(query)}`).then(async response => {
    const apiProducts = response?.apiProduct ?? [];
    ...
    const lastApiProductName = apiProducts.length > 0 ? apiProducts.at(apiProducts.length - 1).name : null;

    if (lastApiProductName && lastApiProductName.length > 0) {
      // Keep fetching until there are no more results\
      return getApigeeEdgeApiProducts(apigeeEdgeClient, callback, lastApiProductName).then(results => apiProducts.concat(results));
    }
