 ...
 * @param {(developer) => Promise<void|null>} callback 
 * @param {string} startKey 
 * @returns 
 */
async function getApigeeEdgeDevelopers(apigeeEdgeClient, callback = null, startKey = null) {
  ...
  return apigeeEdgeClient.get(`/developers?${querystring.stringify(query)}`).then(async response => {
    ...
    if (lastEmail && lastEmail.length > 0) {
      ...
      return getApigeeEdgeDevelopers(apigeeEdgeClient, callback, lastEmail).then(results => developers.concat(results));
    }
...
export default getApigeeEdgeDevelopers;