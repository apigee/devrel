 ...
 * @returns 
 */
async function getApigeeEdgeDeveloperApps(apigeeEdgeClient, callback = null, startKey = null) {
  ...
  return apigeeEdgeClient.get(`/apps?${querystring.stringify(query)}`).then(async response => {
    ...
    if (lastAppId && lastAppId.length > 0) {
      ...
      return getApigeeEdgeDeveloperApps(apigeeEdgeClient, callback, lastAppId).then(results => developerApps.concat(results));
    }
...
export default getApigeeEdgeDeveloperApps;